...
export default function HodBatchUploadPage() {
    ...
    const processSelectedFile = async (selectedFile: File) => {
        const isPdfFile = selectedFile.name.toLowerCase().endsWith(".pdf") || selectedFile.type.toLowerCase().includes("pdf");
        const isCsvFile = selectedFile.name.toLowerCase().endsWith(".csv") || selectedFile.type.includes("csv") || selectedFile.type === "text/plain";
            ...
            <PageHeader
                ...
                breadcrumbs={
                    <div className='flex items-center gap-1 text-sm font-medium'>
                        <Link href='/hod/batches' className='text-muted-foreground hover:text-foreground transition-colors'>
                            My Batches
                        </Link>
                ...
                <div className='grid gap-8'>
                    <Card className='p-6 border-border bg-card/50 backdrop-blur-sm'>
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">Select the program and academic period</p>
                            </div>
                        </div>
                    ...
                    <Card className='p-6 border-border bg-card/50 backdrop-blur-sm'>
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">Upload your CSV or PDF result sheet</p>
                            </div>
                        </div>
                        ...
                        {!file ? (
                            <div
                                ...
                                onDrop={(e) => { e.preventDefault(); setIsDragOver(false); if (e.dataTransfer.files[0]) processSelectedFile(e.dataTransfer.files[0]); }}
                            >
                                <input
                                    type='file'
                                ...
                                <p className='text-xs text-muted-foreground mb-6 max-w-xs mx-auto'>
                                    Supported formats: CSV (standard template) or Official PDF result sheets.
                                </p>
                                ...
                                <div className='flex items-center gap-3'>
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-muted-foreground font-bold uppercase mt-0.5">
                                            {(file.size / 1024).toFixed(1)} KB • {isParsing ? "Processing..." : selectedFileType?.toUpperCase()}
                                        </p>
                                    </div>
                                </div>
                                <Button variant="ghost" size="icon" onClick={resetUploadState} className='rounded-full hover:bg-destructive/10 hover:text-destructive'>
                                    <X className='h-4 w-4' />
                                </Button>
                            </div>
                        )}
                        ...
                        <Card className='p-6 border-border bg-card/50 backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4'>
                             <div className="flex items-center gap-3 mb-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">Extracted data from CSV</p>
                                </div>
                            </div>
                            ...
                            <div className='grid grid-cols-2 md:grid-cols-3 gap-4 mb-6'>
                                ...
                                <div className='rounded-lg border border-border bg-muted/20 p-3'>
                                    <p className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground">Students</p>
                                    <p className="text-lg font-bold">{validation.studentCount}</p>
                                </div>
                                <div className={cn("rounded-lg border p-3 md:col-span-1 col-span-2", validation.errors.length > 0 ? "border-destructive/20 bg-destructive/5" : "border-emerald-500/20 bg-emerald-500/5")}>
                                    <p className={cn("text-[9px] font-bold uppercase tracking-wider", validation.errors.length > 0 ? "text-destructive" : "text-emerald-600")}>Status</p>
                                    <p className={cn("text-lg font-bold", validation.errors.length > 0 ? "text-destructive" : "text-emerald-600")}>
                                        {validation.errors.length > 0 ? `${validation.errors.length} Issues` : "Valid"}
                                    </p>
                                </div>
                            </div>
                            ...
                            {validation.errors.length > 0 && (
                                <div className='rounded-lg border border-destructive/20 bg-destructive/5 p-4 mb-6'>
                                    <h4 className="text-xs font-bold text-destructive mb-2 uppercase">Validation Issues</h4>
                            ...
                            <div className='rounded-lg border border-border overflow-hidden bg-background/30'>
                                <DataTable
                                    ...
                                    columns={[
                                        { header: "Matric", accessorKey: "matricNumber", className: "px-3 py-2 text-xs font-mono" },
                                        { header: "Name", accessorKey: "studentName", className: "px-3 py-2 text-xs font-semibold" },
                                        { header: "Course", accessorKey: "courseCode", className: "px-3 py-2 text-xs" },
                                        { header: "Grade", accessorKey: "grade", className: "px-3 py-2 text-xs", cell: (r: any) => <Badge variant="secondary" className="font-bold py-0">{r.grade}</Badge> }
                                    ]}
                                />
                            </div>
                        </Card>
                    ...
                    {submitSuccess ? (
                            ...
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground mt-1">
                                        Successfully uploaded <span className="font-bold text-emerald-600">{submitSuccess.students} student results</span>.
                                    </p>
                                </div>
                                <div className="flex gap-3 mt-4">
                                    ...
                                    <Button asChild className="rounded-full bg-emerald-600 hover:bg-emerald-700">
                                        <Link href={`/hod/batches/${submitSuccess.batchId}`}>View Batch Details</Link>
                                    </Button>
                                </div>
                            </div>
                        </Card>
                        ...
                        <div className='flex justify-end gap-4 pb-12'>
                            <Button variant="ghost" onClick={resetUploadState} disabled={isSubmitting}>
                                Cancel
                            </Button>
                            <Button 
                                onClick={handleConfirmUpload} 
                                disabled={!file || isSubmitting || isParsing || hasValidationErrors}
