          ...
          <div className='space-y-8 dashboard-section'>
                  ...
                  <div className='flex gap-6'>
                    ...
                    <label className='flex items-center gap-2 cursor-pointer opacity-50'>
                      ...
                      <span className='text-sm text-foreground flex items-center gap-2'>
                        SIS API Sync{" "}
                        <span className='text-[10px] bg-surface-subtle px-1.5 py-0.5 rounded'>
                          Coming Soon
                        </span>
                      </span>
                    </label>
                  ...
                  <label className='flex items-start gap-3 rounded-lg border border-border-subtle bg-surface-subtle/30 p-3 cursor-pointer'>
                    ...
                    <span className='text-sm text-foreground'>
                      <span className='block font-medium'>
                        Send results to parents immediately after upload
                      </span>
                      <span className='text-text-muted text-xs'>
                        When off, batch stays pending and can be dispatched
                        later.
                      </span>
                    </span>
                  </label>
                      ...
                      <p className='text-xs text-text-muted mt-0.5'>
                        ...
                        {isParsing
                          ...
                          : selectedFileType === "pdf"
                            ? "PDF ready for server extraction"
                            : "Ready"}
                      </p>
                    </div>
                  </div>
                ...
                <div className='p-6 border-b border-border-subtle'>
                  ...
                  <div className='space-y-2 mb-6'>
                    <div className='flex items-center gap-2 text-sm text-status-success font-medium'>
                      ...
                      {validation.studentCount} students parsed
                    </div>
                    <div className='flex items-center gap-2 text-sm text-status-success font-medium'>
                      <CheckCircle className='h-4 w-4' /> {validation.rowCount}{" "}
                      course records
                    </div>
                    <div
                      className={`flex items-center gap-2 text-sm font-medium ${validation.errors.length > 0 ? "text-status-danger" : "text-status-success"}`}
                    >
                      ...
                      columns={[
                        {
                          ...
                          className:
                            "px-3 py-2 text-sm font-mono text-text-muted",
                          cell: (row: any) => row.matricNumber || "-",
                        },
                        {
                          header: "Student Name",
                        ...
                        },
                        {
                          ...
                          className: "px-3 py-2 text-sm text-foreground",
                          cell: (row: any) => row.courseCode || "-",
                        },
                        {
                          header: "Unit",
                          accessorKey: "courseUnit",
                          className: "px-3 py-2 text-sm text-foreground",
                          cell: (row: any) => row.courseUnit || "-",
                        },
                        {
                          header: "Grade",
                          accessorKey: "grade",
            ...
            {submitSuccess ? (
              <div className='rounded-lg border border-status-success/40 bg-status-success/10 p-4 text-sm text-status-success'>
                <p>
                  Upload complete for {submitSuccess.students} students. Batch
                  ID: {submitSuccess.batchId}
                </p>
                <p className='mt-1'>
            ...
            <div className='flex items-center justify-end gap-4 pt-4 pb-12'>
              <Link
                href='/admin/batches'
                className='px-5 py-2.5 text-sm font-medium text-text-muted hover:text-foreground transition-colors'
              >
                Cancel
              </Link>
              <button
          ...
          <div className='xl:sticky xl:top-24 h-fit'>
            <div className='rounded-2xl border border-(--border-subtle) bg-(--surface-strong) p-6 shadow-sm'>
              ...
              <p className='text-sm text-text-muted mb-6'>
                Upload either a structured CSV or a combined PDF result sheet.
                CSV supports full preview, while PDF extraction happens on
                upload.
              </p>

              <div className='space-y-4 mb-8'>
                <div>
                  <div className='text-xs font-semibold uppercase tracking-widest text-foreground mb-2'>
                    Required Columns
                  </div>
                ...
                <div>
                  <div className='text-xs font-semibold uppercase tracking-widest text-foreground mb-2 mt-6'>
                    Optional Columns
                  </div>
                  <ul className='text-sm text-text-muted space-y-2 font-mono bg-surface-main p-4 rounded border border-border-subtle'>
                    <li>parent_phone</li>
              ...
              <button
                ...
                className='flex w-full items-center justify-center gap-2 rounded-lg border border-border-subtle bg-surface-main px-4 py-2.5 text-sm font-medium text-foreground hover:bg-surface-subtle transition-colors'
              >
