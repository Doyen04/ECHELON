...
import { Download, Filter, Search, ChevronRight, Upload, Loader2 } from "lucide-react";
...
export default function BatchesPage() {
  ...
  const filteredBatches = useMemo(() => {
      return (batches || []).filter((b: any) => {
          const matchesDept = !selectedDept || b.program?.department?.id === selectedDept || b.department === departments.find(d => d.id === selectedDept)?.name;
          const matchesProg = !selectedProgram || b.programId === selectedProgram || b.program?.id === selectedProgram;
  ...
  const sessions = Array.from(new Set((batches || []).map((b: any) => b.session))).filter(Boolean);
  const levels = Array.from(new Set((batches || []).map((b: any) => String(b.level)))).filter(l => l !== "null" && l !== "undefined").sort();
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6 bg-card/30 p-4 rounded-2xl border border-border">
                ...
                {(selectedDept || selectedProgram || selectedLevel) && (
                    <Button 
                        variant="ghost" 
                        size="sm" 
                        onClick={() => { setSelectedDept(""); setSelectedProgram(""); setSelectedLevel(""); }}
