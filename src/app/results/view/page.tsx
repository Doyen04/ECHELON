...
export default function PublicResultViewPage({
  ...
  if (isLoading) {
    return <LoadingState title='Verifying result link...' />;
  }
  ...
  const primaryGuardian =
    student.guardians.find(
      (guardian: any) => guardian.email || guardian.phone,
    ) ?? student.guardians[0];
  ...
  return (
    <div className='min-h-screen bg-background font-sans text-foreground'>
      ...
      <main className='mx-auto max-w-170 space-y-6 p-4 pb-20 sm:p-6 md:p-8'>
        <div
          className='page-transition-enter space-y-2 py-4 text-center'
        ...
        >
          ...
          <div className='mt-1 text-sm font-medium text-foreground'>
            {result.batch.session} Session •{" "}
            {String(result.batch.semester).toLowerCase()} Semester
          </div>
        </div>
        ...
        >
          ...
          <div>
            <div className='font-medium text-foreground'>
              Officially approved by University Senate
            </div>
            <div className='mt-0.5 text-sm text-text-muted'>
              Published on{" "}
              {formatDateTime(
                result.batch.approvedAt ?? result.batch.uploadedAt,
              )}
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div className='divide-y divide-border-subtle'>
            {courses.map((course) => (
              <div
                key={course.code}
                className='flex flex-col px-4 py-4 transition-colors hover:bg-surface-subtle/20 sm:grid sm:grid-cols-[1fr_2fr_1fr_1fr_1fr] sm:items-center sm:px-6 sm:py-3'
              >
                ...
                <div className='mb-3 text-sm font-medium leading-snug text-foreground sm:hidden'>
                  {course.title}
                </div>
                ...
                <div className='hidden font-mono text-sm font-medium text-brand sm:block'>
                  {course.code}
                </div>
                ...
                <div className='hidden text-right font-mono text-sm text-text-muted sm:block'>
                  {course.score ?? "N/A"}
                </div>
              </div>
            ))}
          ...
          <div className='flex flex-wrap items-center justify-between gap-4 border-t border-border-subtle bg-surface-subtle px-4 py-4 sm:px-6'>
            ...
            <div>
              <span className='mr-2 text-xs uppercase tracking-widest text-text-muted'>
                Semester GPA
              </span>
              <span className='font-serif text-2xl font-medium text-brand'>
                {result.cgpa ?? result.gpa}
              </span>
            </div>
          </div>
        </div>
        ...
        <div
          className='page-transition-enter space-y-1 py-4 text-center'
        ...
        >
          ...
          <p className='text-xs text-text-muted'>
            This link automatically expires on{" "}
            <span className='font-medium text-text-muted'>
              {formatDateTime(portalToken.expiresAt)}
            </span>
            . For queries, contact{" "}
            {primaryGuardian?.email ? (
              ...
              <span className='text-brand'>registry@university.edu.ng</span>
            )}
            .
          </p>
        </div>
