...
/**
 * Handles Mountain Top University style broadsheets where each page is one student,
 * with course codes in one row, units in another, and results in a final row.
 */
/**
 * Handles Mountain Top University style broadsheets where data is interleaved across many lines.
 */
function parseStudentRowsFromMatrixBroadsheet(lines: string[], fallbackDepartment: string): StudentImportRow[] {
    const students: StudentImportRow[] = [];

    // 1. Find Course Status line (it's a very stable anchor)
    const statusLineIdx = lines.findIndex(l => /course\s*status/i.test(l));
    if (statusLineIdx === -1) return [];

    // 2. Extract Course Codes (they are on alternating lines above status)
    const codes: string[] = [];
    let codesSearchIdx = statusLineIdx - 1;
    while (codesSearchIdx >= 0) {
        const currentLine = lines[codesSearchIdx];
        const nextLine = lines[codesSearchIdx + 1];
        
        // If we found a numeric suffix line (like "101") and its prefix line above (like "BIO")
        if (/^\d{3}[A-Z]?$/.test(currentLine) && /^[A-Z]{2,4}$/.test(lines[codesSearchIdx - 1])) {
            // This is a bit complex because BIO 101 are on lines N and N+1
            // But we might have P1, P2, P3 then S1, S2, S3?
            // Actually, based on the RAW output:
            // 9: BIO
            // 10: 101
            // 11: BIO
            // 12: 103
            // So they ARE interleaved: P1, S1, P2, S2...
            // Let's restart the search from line 9 if possible.
            break; 
        }
        codesSearchIdx--;
    }

    // Interleaved Prefix/Suffix search starting from after the header
    let startLine = lines.findIndex(l => /matric\s*no/i.test(l)) + 1;
    if (startLine <= 0) startLine = 5; // Fallback to line 5+ based on raw output

    for (let i = startLine; i < statusLineIdx; i++) {
        const line = lines[i];
        const next = lines[i+1];
        if (/^[A-Z]{2,4}$/i.test(line) && /^\d{3}[A-Z]?$/.test(next)) {
            codes.push(`${line.toUpperCase()}${next.toUpperCase()}`);
            i++; // skip suffix
        }
    }
    ...
    const units = unitsLineIdx !== -1 
        ? lines[unitsLineIdx].split(/\s+/).filter(t => /^\d+$/.test(t)).map(Number)
        : [];

    // 4. Extract Titles (from the bottom of the page)
    const titles: string[] = [];
    const titlesStartIdx = lines.findIndex(l => /previous\s+standing/i.test(l));
    if (titlesStartIdx !== -1) {
        for (let i = titlesStartIdx; i < lines.length; i++) {
            let line = lines[i]
                .replace(/previous\s+standing/i, "")
                .replace(/total\s+units\s+taken.+/i, "")
                .trim();
            
            if (!line || line.length < 3) continue;

            // Split by 2 or more spaces
            const parts = line.split(/\s{2,}/).map(p => p.trim()).filter(p => p.length > 3);
            titles.push(...parts);
            
            // Stop if we hit the system date or footer
            if (/system\s*date/i.test(lines[i])) break;
        }
    }

    // 5. Find Student Records
    // Pattern: serial matric name [prev summaries] score
    const IDENTITY_PATTERN = /^(\d{1,4})\s+([A-Z0-9][A-Z0-9.\/\-]{3,})\s+([A-Za-z][A-Za-z .',\-]+?)\s+\d.*?\s+(\d{1,3})$/i;

    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const match = line.match(IDENTITY_PATTERN);
        if (!match || !isLikelyMatric(match[2])) continue;

        const matricNumber = match[2].trim().toUpperCase();
        const studentName = cleanStudentName(match[3].trim());
        const firstScore = Number(match[4]);

        const studentCourses: StudentImportRow["courses"] = [];
        
        // Results are interleaved on subsequent lines: Grade, Score, Grade, Score...
        // Starting with the first score from the identity line, the next line should be the first grade.
        let cursor = i + 1;
        let score = firstScore;
        
        for (let c = 0; c < codes.length; c++) {
            // Find next Grade
            while (cursor < lines.length && !/^[A-FP]$/.test(lines[cursor].split(/\s+/)[0])) {
                cursor++;
            }
            if (cursor >= lines.length) break;
            
            const grade = lines[cursor].split(/\s+/)[0].toUpperCase();
            cursor++;

            studentCourses.push({
                code: codes[c],
                title: titles[c] || "Matrix Broadsheet Result",
                unit: units[c] || 0,
                grade: GRADE_POINTS[grade] !== undefined ? grade : "N/A",
                score
            });

            // Find next Score (unless it's the last course)
            if (c < codes.length - 1) {
                while (cursor < lines.length && !/^\d{1,3}$/.test(lines[cursor].split(/\s+/)[0])) {
                    cursor++;
                }
                if (cursor >= lines.length) break;
                score = Number(lines[cursor].split(/\s+/)[0]);
                cursor++;
            }
        }

        // Summary: Look for a line containing many GPA-like numbers after the results
        while (cursor < lines.length && !lines[cursor].includes(".")) {
            cursor++;
        }
        
        const summaryLine = lines[cursor] || "";
        // Find all numbers that look like GPA (e.g. 4.04)
        const decimalMatches = summaryLine.match(/\d\.\d{1,2}/g);
        
        const gpa = decimalMatches ? Number(decimalMatches[0]) : 0;
        const cgpa = decimalMatches && decimalMatches.length > 1 ? Number(decimalMatches[decimalMatches.length - 1]) : gpa;

        const departmentLine = lines.find(l => /department\s*:/i.test(l));
        const levelLine = lines.find(l => /level\s*:/i.test(l));
        
        const department = (departmentLine?.split(":")[1]?.trim() ?? fallbackDepartment) || fallbackDepartment;
        const level = Number(levelLine?.split(":")[1]?.trim() ?? "100") || 100;
...
export async function parseStudentRowsFromPdf(pdfBuffer: Buffer, fallbackDepartment: string): Promise<StudentImportRow[]> {
    ...
    // Strategy 1: Matrix Broadsheet (MTU style)
    const broadsheetStudents = parseStudentRowsFromMatrixBroadsheet(lines, fallbackDepartment);
    if (broadsheetStudents.length > 0) return broadsheetStudents;

    // Strategy 2: class result sheet / tabular layout
