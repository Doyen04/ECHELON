  ...
  return (
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {metrics.map((metric, index) => (
        <article
          key={metric.label}
          className="dashboard-card group rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-strong)] px-5 py-5 shadow-[0_20px_40px_-35px_rgba(2,23,23,0.75)]"
        ...
        >
          ...
          <div className="mt-4 flex items-start justify-between gap-3">
            <p className="text-3xl font-semibold leading-none tracking-tight text-[var(--text-primary)]">
              {metric.value}
            </p>
            <span
          ...
          <p className="mt-4 text-sm text-[var(--text-muted)]">{metric.helper}</p>
        </article>
      ))}
    </section>
  );
